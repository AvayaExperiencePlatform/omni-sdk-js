...
[...document.getElementsByClassName("dtmf") as HTMLCollectionOf<HTMLButtonElement>]
	.forEach(button =>
		button.onclick = () => call?.sendDTMF(button.value as AxpDtmfTone)
	);
...
function updateAvailableDevicesList(mediaInterface: AxpMediaInterface, deviceListContainer: HTMLSelectElement) {
	...
	deviceListContainer.onchange = () => {
		const device = mediaInterface.getAvailableDevices().find(device => device.getDeviceId() === deviceListContainer.value);
		if (device) {
